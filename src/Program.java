...
Розробити модульні тести до 5 методів своєї моделі з лабораторної роботи № 3
з використанням технік аналіза граничних значень та розбиття вхідних даних на класи еквівалентності.
Продемонструвати використання слідуючих анотацій:
@ParameterizedTest, @ValueSource, @MethodSource, @CsvSource, @Test, @DisplayName
                Питання до захисту лабораторної роботи № 6:
-	в чому полягає головна мета тестування
Основною метою тестування є пошук помилок у програмі. На етапі розробки тестування допомагає виявити та
виправити наявні помилки. Код варто тестувати перед переданням його замовнику, оскільки чим менше помилок
знайдут користувачі, тим краще створена та протестована програма.
-	техніки тестування(аналіз граничних значень та  розбиття вхідних даних на класи еквівалентності)
Розбиття вхідних даних на класи еквівалентності передбачає, що якщо є інтервал допустимих значень і робота
програми відповідає очікуванням на одному з елементів інтервалу та на одному з елементів поза інтервалом,
то програма працює правильно.
Якщо елемент, який тестують має певні границі, то якщо програма поводить себе відповідно очікуванням
при роботі з максимальною та мінімальною границею та зі значеннями, що більше і менше границь,
то програма працює вільно.
-	базові можливості JUnit 5.4 для модульного тестування
JUnit5 - фреймворк, що допомагає створювати модульні тести, тобто створити тестову сцену та керувати життєвим
циклом тестів. Можна створювати параметризовані тести, тобто тести, які повторюються декілька разів з різними параметрами.
Ппраметри можна передавати як в парі з очікуваним значенням(@CsvSource), так і самі по собі(@ValueSource). Також можна
використати метод, що буде повертати необхідну кількість параметрів(@MethodSource).
*/

//----------КОД З ЛАБОРАТОРНОЇ РОБОТИ №3 --------//
import javax.swing.*;
...
class PrivateParking {
    ...
    public boolean ParkCar(Car car, int placeForParking, LocalDate date, LocalTime time){
    ...
    public boolean LeaveParking(int placeForParking, LocalDate date, LocalTime time){
        if(placeForParking > volume) {
            System.out.println("Даного місця на парковці не існує!");
            return false;
        }else{
    ...
    public ParkingPlace getParkingPlace(int number){
        if(number <= volume){
...
class Journal{
    ...
    //ФОРМУВАННЯ ЗВІТУ
    public double CreateOrder(String owner, float price, LocalDate start, LocalDate end){
        ...
        //Не формувати звіт для власника авто, якого немає в записах
        if(this.SearchByOwner(owner).isEmpty()){
            return 0;
        }
        //Не формувати звіт, яещо вказана від'ємна ціна
        if(price < 0 ){
            return 0;
        }
        ...
        //Не повертати рахунок, якщо запаркувалися після того, як виїхали
        if(totalSum > 0){
            return totalSum;
        }else{
            return 0;
